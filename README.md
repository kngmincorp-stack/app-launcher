...
- **パネル管理**: 「＋ アプリを追加」で追加、右クリックで 起動 / 停止 / 編集 / 削除 / 並べ替え。
- **右クリック「停止」**: 登録パスに厳密一致するプロセスだけを強制終了（確認ダイアログ付き）。
  同名 exe が別の場所で動いていても誤って止めない。onefile アプリの親子 2 プロセスも両方停止。
- **タスクトレイ常駐**: ×ボタンで終了せずトレイへ。トレイメニュー「開く / 終了」。
- **Windows スタートアップ登録**: チェック 1 つで PC 起動時に自動実行（ウィンドウ表示のまま開始）。
